...
public class BsonQueryFactory implements QueryFactory {
    ...
    private int endOfValueIndex(String query, int index) {


        int indexOfNextComma = query.indexOf(",", index);
        int indexOfNextRightBracket = query.indexOf("]", index);
        int indexOfNextRightBrace = query.indexOf("}", index);

        if (indexOfNextComma != -1 && indexOfNextRightBracket != -1) {
            return min(indexOfNextComma, indexOfNextRightBracket);
        } else if (indexOfNextComma == -1 && indexOfNextRightBracket == -1) {
            return query.length() - 1;
        } else if (indexOfNextComma != -1) {
            return indexOfNextComma;
        } else {
            return indexOfNextRightBracket;
        }
    }
